...
class VoiceHandler:
    ...
    def start_voice_interaction(self):
        ...
        speak_and_wait("Good day, sir. How may I assist you today?")
        print("🎧 TTS finished - now listening for user...")
        self.listen_for_command()

    def start_voice_interaction_with_callback(self):
        """Alternative callback-based voice start"""
        print("🎤 Starting voice interaction with callback...")
    ...
    def process_command(self, command):
        ...
        if any(word in command_lower for word in ["open", "launch", "start", "run"]):
            if launch_app_by_voice(command):
                return
            else:
                speak_and_wait("Which application would you like me to open, sir?")
                self.listen_for_command()
                return

        # Quit/close/exit/stop commands
        elif any(word in command_lower for word in ["quit", "close", "exit", "stop"]):
            if quit_app_by_voice(command):
                return
            else:
                speak_and_wait("Which application would you like me to close, sir?")
                self.listen_for_command()
                return

        # Running apps inquiry
        elif any(phrase in command_lower for phrase in ["what's running", "running apps", "what apps", "show running"]):
            if self.app_launcher:
                speak_and_wait("Let me check your running applications, sir.")
                self.app_launcher.speak_running_apps()
            else:
                speak_and_wait("I cannot check running applications at the moment, sir.")
